...
const ACTION_TYPES = [
  ...
  'Responder-directed stabilization action',
...
Escalation Required: ${
      lifecycleDecision.shouldEscalate ? 'YES' : 'NO'
    }
Recovery Monitoring Required: ${
      lifecycleDecision.shouldMonitorRecovery ? 'YES' : 'NO'
    }
Command Visibility: ${
      lifecycleDecision.commandVisibility ? 'YES' : 'NO'
    }
...
Governance Boundary:
This record confirms whether a routed case converted into governed stabilization action. It does not verify final outcome, declare recovery, or close the case. Outcome verification belongs to /outcomes. Recovery durability belongs to /recovery.

Non-Punitive Statement:
This action evidence record does not blame a person, team, family, responder, institution, or partner. It preserves operational movement, remaining risk, ownership posture, review need, and continuity visibility.
  ...
  async function saveInterventionEvidence() {
    ...
    const {
      ...
      .insert({
        ...
        } Action status: ${actionStatus}. Continuity risk: ${continuityRisk}. Evidence level: ${evidenceLevel}. Owner posture: ${ownerPosture}. Review window: ${reviewWindow}.`,
        actor:
      ...
      <div style={styles.container}>
        <section style={styles.hero}>
          <p style={styles.kicker}>
            TSINAXA CGI • INTERVENTION EVIDENCE
          </p>
          ...
          <p style={styles.subtitle}>
            Convert routed instability into
            structured action evidence. This
            surface records what action was taken,
            who owns the movement, what evidence
            exists, what risk remains, and when
            governance review is required.
          </p>

          <div style={styles.boundaryBox}>
            <strong>Boundary:</strong>{' '}
            /interventions records governed
            action evidence only. It does not
            verify outcomes, declare recovery,
            or close continuity risk.
          </div>
        </section>
        ...
        <section style={styles.layoutGrid}>
          <div style={styles.card}>
            ...
            <p style={styles.panelNote}>
              Use this only after a routed case
              has received a real stabilization
              action. Record operational facts,
              ownership posture, evidence
              strength, residual risk, and review
              timing.
            </p>
            ...
            >
              {loading
                ? 'Saving Evidence...'
                : 'Save Stabilization Action Evidence'}
            </button>
          </div>
          ...
          <div style={styles.card}>
            ...
            <p style={styles.panelNote}>
              This is the governed action
              evidence record that will be saved
              to the intervention table,
              preserved in the timeline, and
              carried into lifecycle governance.
            </p>

            <pre style={styles.summaryBox}>
              {interventionEvidence() ||
                'Select a CGI case to generate governed stabilization action evidence.'}
            </pre>
          </div>
        </section>
      </div>
...
async function preserveInterventionGovernanceEvidence(
    ...
    .insert({
      ...
      details: {
        ...
        owner_posture:
          input.ownerPosture,
        residual_risk_note:
          input.residualRiskNote,
        review_window:
          input.reviewWindow,

        stabilization_movement_score:
          Number(
            input.stabilizationMovementScore
          ),
        continuity_risk_after_action:
          input.continuityRisk,
        ...
        recovery_monitoring_required:
          input.lifecycleDecision
            .shouldMonitorRecovery,
        command_visibility_required:
          input.lifecycleDecision
            .commandVisibility,

        additional_governance_notes:
          input.additionalGovernanceNotes.trim() ||
          null,
  ...
  if (error) {
    console.error(
      'Intervention governance evidence logging failed',
      {
...
function buildInterventionSurvivabilityContext(
  input: {
    caseItem: BeneficiaryCase
    actionStatus: string
    continuityRisk: ContinuityRisk
