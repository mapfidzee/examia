...
export default function GovernanceConsolePage() {
  ...
  async function changeResponderStatus(profile: TeacherProfile, nextStatus: string) {
    const reason = window.prompt(
      `Governance reason for moving ${profile.full_name} to ${nextStatus}:`
    )

    if (reason === null) return

    setActionLoading(`${profile.id}-${nextStatus}`)
    setMessage('')

    const previousStatus = profile.status || 'PENDING'
    ...
    const { data: responderData, error: responderError } = await supabase
      ...
      .upsert(
        {
          source_teacher_profile_id: profile.id,
          full_name: profile.full_name,
          email: profile.email,
          operational_status: nextStatus,
          governance_role: 'RESPONDER',
          response_domains: profile.subjects || [],
          learner_levels: profile.grade_levels || [],
          languages: profile.spoken_languages || [],
          region: profile.province || '',
          trust_score: trustScoreForStatus(nextStatus),
          safeguarding_status:
            nextStatus === 'ACTIVE' || nextStatus === 'VERIFIED'
              ? 'ACCEPTED'
              : 'PENDING',
          verification_summary: profile.bio || '',
    ...
    if (responderError) {
      ...
      alert('Status changed, but responder registry could not be updated.')
      setActionLoading(null)
      await loadProfiles()
      return
    }
    ...
    const { error: actionError } = await supabase.from('governance_actions').insert({
      responder_id: responderData?.id || null,
      teacher_profile_id: profile.id,
      action_type: `STATUS_CHANGE_TO_${nextStatus}`,
      previous_status: previousStatus,
      new_status: nextStatus,
      governance_actor: 'EXAMIA Governance Console',
      reason: reason.trim() || 'No reason entered',
      notes: `Responder moved from ${previousStatus} to ${nextStatus}.`,
    })

    if (actionError) {
      console.error(actionError)
      alert('Responder updated, but governance action log failed.')
    }

    setMessage(`Responder moved to ${nextStatus}. Governance action logged.`)
    setActionLoading(null)
    await loadProfiles()
  }
  ...
  const restricted = profiles.filter(
    (p) => p.status === 'RESTRICTED' || p.status === 'SUSPENDED'
  ).length
    ...
    <main style={styles.page}>
      <div style={styles.container}>
        <section style={styles.hero}>
          <p style={styles.kicker}>EXAMIA LIS • GOVERNANCE ENGINE</p>
          ...
          <p style={styles.subtitle}>
            ...
            suspension, removal, and governance evidence for EXAMIA Learning
              ...
              {profiles.map((profile) => (
                <article key={profile.id} style={styles.profileCard}>
                  ...
                  <details style={styles.details}>
                    ...
                    <pre style={styles.bio}>{profile.bio || 'No verification evidence submitted.'}</pre>
                  </details>
                  ...
                  <div style={styles.actionGrid}>
                    {STATUS_FLOW.map((status) => (
                      <button
                        key={status}
                        onClick={() => changeResponderStatus(profile, status)}
