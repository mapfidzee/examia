...
function OutcomesContent() {
  ...
  const [verificationTrajectory, setVerificationTrajectory] = useState('VARIABLE_STABILITY')
  ...
  const survivabilitySignal = buildSurvivabilitySignal({
    ...
    variablePressure: outcomePressure.variableVerification,
  })
  ...
  const verificationPressureMeaning = buildVerificationPressureMeaning({
    variableVerification: outcomePressure.variableVerification,
...
${selectedCase ? lifecycleDecision.nextStatus : 'Awaiting stabilization verification assignment.'}

CASE SIGNAL
${selectedCase?.beneficiary_name || 'Executive synthesis will activate after continuity case selection.'}

STABILITY DOMAIN
${selectedCase?.support_domain || 'Continuity domain visibility pending case assignment.'}

CURRENT CONTINUITY STATUS
${selectedCase?.case_status || 'Verification continuity posture pending operational review.'}
  ...
  const continuityProfiles = [
    {
      ...
      value: 'No concentrated destabilizing verification pattern currently visible.',
    },
    {
      title: 'Verification Observation Load',
      value: 'Outcome verification activity remains within manageable continuity thresholds.',
    },
    {
      title: 'Recurrence Visibility',
      value: 'No active recurrence concentration currently requiring executive escalation.',
    },
    {
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mt-4 rounded-3xl border border-neutral-800 bg-neutral-900/70 p-6 shadow-2xl">
          ...
          <p className="mt-3 max-w-5xl text-sm leading-6 text-neutral-300">
            ...
            remaining variable, weakening, recurring, or becoming eligible for
            recovery durability observation.
          </p>
          ...
          <p className="mt-4 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4 text-sm leading-6 text-cyan-100">
            ...
            stabilization impact. It does not declare durable recovery, erase
            structural continuity memory, or close survivability visibility
            automatically.
          </p>
        </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_1.1fr]">
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900 p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-neutral-400">
              Use this after stabilization action has occurred. Preserve continuity
              credibility, recurrence visibility, survivability meaning, recovery
              eligibility, and executive continuity interpretation.
            </p>
            ...
            <div className="mt-6 space-y-5">
              ...
              <button
                ...
                disabled={loading}
                className="w-full rounded-xl bg-cyan-400 px-4 py-3 text-sm font-semibold text-neutral-950 transition hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {loading
          ...
          <section className="rounded-3xl border border-neutral-800 bg-neutral-900 p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-neutral-400">
              This synthesis confirms whether stabilization movement is
              strengthening, remaining variable, recurring, weakening, escalating,
        ...
        <section className="mt-8 rounded-3xl border border-neutral-800 bg-neutral-900 p-6">
          ...
          <p className="mt-4 text-sm leading-7 text-neutral-300">
            ...
            recurrence, escalation concentration, or weakening trajectory, the
            system should support measured continuity confidence while preserving
            structural memory and executive traceability.
          </p>
        </section>
      </section>
    </main>
...
function buildStabilizationConfidence(input: {
  ...
  if (input.verificationResult === 'RECURRENCE_DETECTED') return 'WEAKENING'
  if (input.verificationTrajectory === 'VARIABLE_STABILITY') return 'VARIABLE'
...
function buildSurvivabilitySignal(input: {
  ...
  if (input.verificationResult === 'ESCALATION_REQUIRED' || input.escalationPressure > 0) {
    return 'SURVIVABILITY_PRESSURE_RISING'
  }

  if (input.recurrenceSignal === 'REPEATED_RECURRENCE' || input.recurrencePressure > 0) {
    return 'RECURRENCE_REQUIRES_VISIBILITY'
  }

  if (input.variablePressure > 0 || input.verificationTrajectory === 'VARIABLE_STABILITY') {
    return 'VARIABLE_STABILITY_REQUIRES_OBSERVATION'
  }

  if (input.recoveryReadiness === 'RECOVERY_WATCH_ELIGIBLE') {
...
function buildVerificationPressureMeaning(input: {
  ...
  if (input.variableVerification > 0) {
    signals.push('variable verification conditions remain visible')
  }

  if (input.recurrenceDetected > 0) {
    signals.push('recurrence visibility remains active')
  }

  if (input.escalationRequired > 0) {
    signals.push('escalation pressure remains operationally visible')
  }
